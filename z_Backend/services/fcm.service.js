...
const initializeFirebase = () => {
  ...
  try {
    ...
    console.warn('⚠️ WARNING: Server running in MOCK FCM mode. Notifications will be logged but not sent.');
...
const sendPushNotification = async (pushToken, title, body, data = {}) => {
  try {
    if (!firebaseInitialized) {
      console.log('⚠️ [MOCK] Would send push notification:', { title, body, token: pushToken });
      return { success: true, messageId: 'mock-message-id', isMock: true };
    }
...
const sendMulticastNotification = async (pushTokens, title, body, data = {}) => {
  try {
    if (!firebaseInitialized) {
      console.log('⚠️ [MOCK] Would send multicast notification:', { title, body, count: pushTokens?.length });
      return { success: true, successCount: pushTokens?.length || 0, failureCount: 0, isMock: true };
    }
